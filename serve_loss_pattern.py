...
def display_serve_loss_pattern(df):
    """
    自分のサーブで失点したパターンをStreamlitのUIに表示する関数
    
    Args:
        df (pd.DataFrame): 試合の得失点データ
    """
    required_columns = ['開始時刻', '得失点の種類', 'ゲーム数', '誰のサーブか', '得点者', 
                        'サーブの種類', 'サーブのコース', 'サーブの質', '失点の内容', 'YouTubeリンク']

    if df.empty or not all(col in df.columns for col in required_columns):
        st.warning('スプレッドシートの読み込みに失敗したか、必要な列が見つかりませんでした。')
        return

    with st.expander('サーブ時の失点パターン一覧'):
        try:
        ...
        if filtered_df.empty:
            st.warning('自分のサーブで失点したパターンは見つかりませんでした。')
            return
        
        display_columns = [
            '開始時刻', 'ゲーム数', 'サーブの種類', 'サーブのコース', 'サーブの質', '失点の内容'
        ]
        ...
        st.info('この表は、自分のサーブで自分が失点した時の各パターンを表示しています。')


def get_serve_loss_pattern_for_ai(df):
    """
    自分のサーブで失点したパターンを抽出し、AIに渡すためのMarkdown文字列を生成する
    ...
    try:
        ...
        return "サーブ時の失点パターンデータ生成中にエラーが発生しました。"
    ...
    if filtered_df.empty:
        return "自分のサーブで失点したパターンは見つかりませんでした。"
    
    # AIに渡すための列を選択
    columns_for_ai = [
        'ゲーム数', 'サーブの種類', 'サーブのコース', 'サーブの質', '失点の内容'
    ]
    
    # Markdown形式に変換
    pattern_markdown = filtered_df[columns_for_ai].to_markdown(index=False)
    
    return f"## 自分のサーブでの失点パターン一覧\n\n{pattern_markdown}"
